function pesquisar() {
  ...
  for (let pais of paisesUsinas) {
    ...
    tagsPais = pais.tags.toLowerCase();

    // Converte as tags do pais para uma lista
    listaTags = tagsPais.split(",").map(tag => tag.trim())

    // Itera sobre a lista de tags para encontrar a igualdade com o temo de pesauisa
    for (let i = 0; i < listaTags.length; i++) {

      // Verifica se as tags do país são iguais ao termo de pesquisa
      if (listaTags[i] == campoPesquisa) {
        ...
        resultados += `
          <div class="item-resultado">
            ...
            <a href=${pais.fonte} target="_blank"><br>Fonte dos dados<br><br></a>
